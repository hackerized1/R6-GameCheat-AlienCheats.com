...
inline auto __fastcall setup_window() -> void
{
	...
	MyWnd = CreateWindowExA(0, wClass.lpszClassName, ("Discord"), WS_POPUP | WS_VISIBLE, GameRect.left - 2.f, GameRect.top - 2.f, globals.width + 2.f, globals.height + 2.f, 0, 0, 0, 0);
...
void RenderMenu()
{
	...
	if (globals.menu)
	{
		...
		ImGui::Begin(("Discord Hong"), (bool*)0);
		{
			ImGui::Text("Visuals");
			ImGui::Spacing();
			ImGui::Checkbox(xorstr_("Enable Aimbot"), &globals.aimbot);
			if (globals.aimbot)
			{
				ImGui::SliderFloat(xorstr_(" "), &globals.smooth, 0, 25, "Smooth %.0f%%");
				ImGui::SliderFloat(xorstr_("  "), &globals.aimFOV, 0, 250, "Fov %.0f");
			}
			...
			if (ImGui::Button("Exit", ImVec2(150, 150)))
			{
				delete_service("HongBoss");
				Sleep(1000);
				exit(0);
			}
			ImGui::Spacing();
			...
			ImGui::Checkbox("Fov", &globals.showfov);
